...
| OMR model (fallbacks) | **CRNN+CTC (PrIMuS-based)**, then **YOLOv8 glyph detection** + heuristic decoder | **RETIRED (2026-07-07)** — Rung 1.5 retired the export/size concern; the Rung-2 PASS (99.9% headline accuracy — `docs/METRICS.md`) retired the accuracy concern. Kept here for the record only. |
...
- Training mechanics to scaffold (developer is new to this):
  ...
  - **Sanity check first: overfit 10 samples** (nothing frozen, on the Mac/MPS) to confirm the data,
    ...
    diagnostic**; the real run restarts from the original pretrained weights. ✅ **GO (2026-07-02)** — `docs/log/status-log.md`.
  - **ONNX/browser gate before scaling** (`docs/PHASE2.md` §5 Rung 1.5): export to ONNX + decode one
    ...
    ✅ **PASS (2026-07-03)** — `docs/log/status-log.md`.
...
- **ONNX export / in-browser inference of an autoregressive encoder-decoder** — the product's
  ...
  GPU training. ✅ **RESOLVED (2026-07-03): the Rung-1.5 gate PASSED** — `docs/log/status-log.md`; this risk is
...
> **Status and next action now live in [docs/STATUS.md](docs/STATUS.md).** This section used to be
> the canonical one; it grew into a 680-line append-log and was split on 2026-07-26.
>
> - current state + next action → [docs/STATUS.md](docs/STATUS.md)
> - every measured number → [docs/METRICS.md](docs/METRICS.md)
