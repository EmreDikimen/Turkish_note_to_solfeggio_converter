...
**Phase 2: DONE (2026-07-02 … 2026-07-06).** (Rungs 0–1.5 of the `docs/PHASE2.md` ladder + the
Rung-2 dataset/training kit = Phase 2; the Rung-2 scaled fine-tune below formally opens **Phase 3** —
see the boundary note above §3's Phase 2. The list runs straight on, oldest first, so the ✅ entries
from "Rung 2 — PASS" downward are Phase-3 work.)

**Phase 3: IN PROGRESS (opened 2026-07-07 with the Rung-2 fine-tune; real-data track since
2026-07-10).** Synthetic accuracy is solved (99.9% headline on held-out synthetic pieces); all
remaining work is about REAL pages — collection, labeling, the frozen exam, and the Round-N
train/measure loop. Live detail: `docs/RUNG3.md`; plain-English summary: `docs/OVERVIEW.md`.
...
- ✅ **Round-1 init A/B DONE (2026-07-22) — Arm A (TWO-STAGE) WINS, 89.2% vs 78.4% real-val mean
  AEU F1** (`docs/RUNG3.md` Step 4.2; full numbers `src/vision/MODEL_EVAL.md`):
  - **Arm A (winner)** = stage 1 synthetic-only 6k steps from BASE → stage 2 from stage-1 `best`,
    2k steps @ lr 1e-5 with the real pools oversampled **`:8`** (real = 33.3% of the pool).
    **Arm B (control)** = single-stage joint, 7k steps, real at its natural 5.9%. Tie-break not
    needed (both 1.6% arc-triggered false-`\tup3`). Selection ran on the merged 271-strip real-val
    pool built by the new `src/vision/make_realval_pool.py`.
  - **THE TUP3 CATASTROPHE IS FIXED: `\tup3` precision 15.1% → 97.4%; arc-triggered false-`\tup3`
    77.6% → 1.6%** (floor ≤10%) — the slur distractors did their job. `\sig`/`\sigend` F1 96–98%
    validates the conventional-signature work.
  - **⚠ The margin is LOW-N driven, not decisive:** `\komaSharp` has **1 gold** in real-val and
    `\kucukSharp` 21; together they account for 10.4 of the 10.8 pp gap. On the four ≥30-gold
    classes the arms are effectively tied (92.7% vs 92.2%). Real signal = `\kucukSharp` recall
    95.2% vs 61.9%, plus A's better source consistency (0.6 vs 2.8 pp) and tup3 precision.
  - **NOT comparable to the 64.1% AEU / 57.0% F1 baseline** (that was exam v2.1, 352 strips; this is
    real-val, 271). Real-val is also the SELECTION set, so its Step-4.0-floor pass is indicative
...
- ✅ **Plan-review addenda ADOPTED (2026-07-23, `docs/RUNG3.md` Step 4.4a)** — amendments to the
  ...
  `round1-best` **bridge read** as its baseline; (8) ⛔ **pivot trigger DROPPED by the user
  (2026-07-23) — there is NO pre-registered pivot trigger** (this entry previously said "RATIFIED",
  contradicting `docs/RUNG3.md` Step 4.4a item 8; **RUNG3.md is authoritative**, corrected here
  2026-07-25). The proposed rule (Round-2 exam closing < half the remaining gap to the 80% F1 floor
  ⇒ correction-loop pivot becomes primary) was dropped because it inherited an unproven premise
  (that Round-2 levers are ≥ Round-1's, when Round 1's gain was partly a one-time synthetic→real
  jump) and keyed a six-floor ship decision on one metric. **The correction-loop pivot stays a
  SITUATIONAL call** after the Round-2 exam, not a bound number; consequence accepted = no automatic
  guard against a sunk-cost "one more round"; (9) logged only: consider tie-token loss masking on
  noisy-pool strips.
- ✅ **Round-1 synthetic re-render BUILT (2026-07-21) — and the ordering below is SUPERSEDED**
  ...
  - **ORDERING CHANGE: Round 1 runs FIRST; the additive-only re-slice moves to ROUND 2.** Re-slice
    adjudication cost is driven by decoder quality (SymbTr is the label; the review queue is
    decode-vs-SymbTr disagreements), and the current labeler is **neyzen-only (362 strips)** while
    the re-slice target is **nota-dominant** — so re-slicing now would manufacture false disputes.
    The Round-1 model becomes a nota-aware labeler and makes that queue smaller and cleaner. Round 1
    doesn't need the recoveries (its levers are the re-render + the promoted ~2,351 real strips), and
    it is an A/B *experiment*, not the maximal final model. Round-1 data scope is FROZEN.
  - **Re-render design (locked):** carry mode (`measure`) **replaces keysig** and is dominant, at
    **transpose 0 only** so the conventional makam signature matches the notation, bulked via
    `CARRY_PASSES=4` seeded augmentation passes; **every mode** is the minority and carries the
    **transpose augmentation** (t≠0, no signature). `stripExport.ts` gained a carry branch (all
    chunks, `\sig` prefix on row-start only — matching how REAL carry strips are labelled).
  - **Per-makam conventional PRINTED signatures**: `data/makam_signatures.json` +
    `scripts/build_makam_signatures.py`, built from the **adjudication-confirmed `\sig` blocks in the
    promoted real labels** (theory only as fallback), **variants UNCAPPED** (hicaz 4, şehnaz 4,
    nisaburek 3), all 49 corpus makams. Fed to BOTH the drawn glyphs and the labels (pixels==labels).
  - **Slur distractors** (`drawSlurArc`): label-free arcs ≥3 notes with **no "3"** over a seeded ~35%
    of non-tuplet runs — the fix for "any arc ⇒ `\tup3`" (baseline tup3 precision 15%, arc-triggered
    false-tup3 77.6%). Verified pixels-only (15 drawn w/ seed vs 0 without; labels byte-identical).
  - **Corpus `data/synthetic/strips_v3`: 38,091 strips, 73.3% carry, 49 makams, 33 signature
    variants; budget gate PASS (57 ids, cap 59).**
  - **✅ DECIDED (2026-07-21) — accidental distribution → train-time `--every-share` sweep**
    (full pre-registration in `docs/RUNG3.md` Step 4.1). The finding: carry mode matches real
    (**0.36 vs 0.32** inline accidentals/strip ✅) but `every` is 26.7% of strips and **81% of all
    inline accidentals** (4.22/strip = 13× real), making the effective rate **4.4× real** — a
    plausible driver of the komaSharp/komaFlat **hallucination** (precision 21%/54%; Step-4.0
    floor ≥70%). The decision: `--every-share` in `train.py` (per-epoch `WeightedRandomSampler`,
    train-time because strips_v3 is a superset — free, tunable, reversible vs a ~75-min one-guess
    re-render), **default 0.15**, pre-registered arms **{26.7%, 15%, 5%}** — three because the
    inline-rate and class-mix hypotheses make OPPOSITE predictions at 5%, and the winning
    mechanism decides the Round-2 re-render design. **Sequential run budget (4 runs, not the
    6-run cross):** init A/B first at fixed s=0.15, then {26.7%, 5%} on the winning recipe;
    selection stays the ONE locked number (real-val mean AEU F1, tie-break arc-false-`\tup3`).
    Pre-registered confound: `s` also scales transpose exposure (every = sole t≠0 carrier), so a
    5%-arm loss is not attributable between mix-deviation and transpose starvation. Diagnostic
    (not selection): per-class val precision across arms — komaSharp's share RISES as s falls
    (5.7→7.0% vs 2.1% real); if its precision doesn't move with `s`, the hallucination isn't
    distributional and Round 2 needs a different lever. The earlier
    **"komaSharp/kucukSharp boost" item is OVERTURNED by data** (komaSharp already over-represented
    and precision-bound; kucukSharp already matches real); the real gap is **kucukFlat**, whose
    residual is a **makam-mix** artifact, so the `bakiyeSharp→kucukFlat` respell is **held**.
- ⏳ **Next (Round-1 plan refined 2026-07-20, `docs/RUNG3.md` Step 4; ORDERING BELOW SUPERSEDED by
  the 2026-07-21 entry above — re-slice is now ROUND 2; selection
