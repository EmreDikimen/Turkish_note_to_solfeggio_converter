...
|---|---|
| **Cold start** | **10.6 s** to a first answer, of which **9.5 s is loading the three graphs** (against 1.5 s on the M4). Container start is the small half. ⚠ **A cold start does not cost a friend 10.6 s of waiting — it costs them the server path entirely.** The container listens before its graphs are loaded, so a decode arriving in that window gets the server's truthful `503 model still loading`, and `remote.ts` treats any error as "fall back" with no retry: the whole page is then read on their own machine (**+211 MB of weights on a first fallback**). Measured on the deployed site 2026-08-08, in the row below |
...
| **⚠ `--cpu-boost` did NOT visibly help — TWO readings now** | `loadMs` **25,857 ms** (revision `…00003-jrl`) and **25,216 ms** (`…00004-nc2`, an independent container start) against **9,500 ms** before the flag. Two consistent readings weaken the first explanation — lazy image-layer streaming on a freshly pushed image would be expected to fade on the second revision, and it did not. **Still not a finding**: the pre-boost side is a single reading, the revisions differ in more than the flag, and nothing was held constant on purpose. What it does say is that **the flag has not earned its place**, and it was taken because it was free on a redeploy rather than on evidence. Worth one controlled read (boost off, same image) before it is trusted or removed |
| **⚠ A genuine cold start is STILL not measured** | The 2026-08-06 attempt (18 min idle, then `/health`) **did not capture one**: the reply came back in 0.26 s with `uptimeS` 315, i.e. a warm instance that had started ~5 min earlier without being asked. The honest statement remains **10.6 s, measured minutes after a deploy**. Measuring it properly needs container-start timestamps from the logs rather than a wall-clock guess at when Cloud Run scales to zero |

⚠ **Still not measured:** behaviour with two friends uploading at once. A cold start after real idle
was in this list and is being read now — see the row above for why the redeploy's number cannot
settle it.
