...
1. **Init = an EXPERIMENT, not a decree (the highest-leverage choice).** The pre-registered
   ...
   `--every-share 0.15`; the every-share sweep then runs on the winning recipe (Step 4.1,
...
**⚠ AMENDED 2026-07-22, before any sweep run** (full text: "Every-share sweep protocol" under
Step 4.2): sweep selection = mean AEU F1 over the four **≥30-gold** real-val classes only (the
...
**⛔ CANCELLED later the same day, still before any sweep run** (decision block under Step 4.2):
...
after the §1c extension) — watch whether the winner's recall dip persists there.
...
**Pre-registration inconsistency logged, not silently resolved:** Step 4.0's table floors classes
with **≥20 gold** (so `\komaSharp` at 18 carries none), while the sweep amendment referred to "the
exam's ≥70% komaSharp-precision floor". **Step 4.0 is binding**; komaSharp is scored and reported,
floor-free.
...
1. **Photo test (free, could change everything).** Real users take phone photos (shadows, angles,
   ...
   (`00_ALL_25_MERGED.pdf`), photograph them (see Step 4.5), and run the model. If the photo score is
...
> **Added 2026-07-23 (details in Step 4.4a):** before those two checks, a few free tasks run first —
